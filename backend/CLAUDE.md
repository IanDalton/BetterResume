...
- `model_routing.py` — every OpenRouter request carries `provider.require_parameters: true`, `reasoning: {enabled: false}` and (from the structured output) `tool_choice: "required"`. `require_parameters` turns the other two into hard routing filters that plenty of good models cannot satisfy: 83 of 333 tool-capable models take no `reasoning` parameter, so disabling it leaves no eligible endpoint (404 "no endpoints found that can handle the requested parameters"); others mandate reasoning (400); and some accept `tool_choice: "auto"` only (404 naming `tool_choice`). None of it is visible in the catalog. `prepare()` wraps every `openrouter:` model string in `AdaptiveRoutingModel`, which retries the failed request with one demand dropped and remembers the concession for the rest of the process. Both demands are optimisations, not correctness requirements -- the agent's output validator still rejects an answer that skipped its tools. Adaptive rather than stored, so it self-heals across workers and covers models that never pass through the dashboard (eval runs pick arbitrary catalog models).
- `model_probe.py` — `probe_model`: one minimal request in our exact shape (structured output + a function tool, plus the OpenRouter settings), run through the same `model_routing.prepare()` path as production, with the same output-retry budget. Reports what the model needs: nothing, one or both routing concessions (`concessions`), or it does not work. The admin `PUT /model-config` runs it on the primary and fallback before storing them (rejecting only the third case); `POST /model-check` exposes it standalone. A model can advertise `tools`/`tool_choice` in the OpenRouter catalog and still refuse `tool_choice: "required"`, so a live request is the only reliable check.
- `agent.py` — module-level pydantic-ai `Agent` singletons: `generation_agent` (tools + structured output), `translation_agent` (no tools), and `resume_import_agent` (structured extraction). No model is bound at construction; the `generate()` / `translate()` / `extract_resume_fields()` module functions resolve one per run (default `DEFAULT_MODEL`), so importing never needs credentials. `ResumeDeps` dataclass carries user_id/vector_store/db into tools. `search_experience` takes a *list* of concepts and searches them concurrently, grouped by query and deduplicated: one search per model request is what let an unforced tool choice run up 50 requests without ever writing the resume. It coerces a bare string and a JSON-encoded array (both observed from real models) rather than spending a request on a validation retry. Runs are capped at `REQUEST_LIMIT` (20, vs pydantic-ai's default 50), and `UsageLimitExceeded` joins `UnexpectedModelBehavior` in the Layer 2 catch so a model that never converges hands off to the fallback instead of failing the user. Forced retrieval (the old `tool_choice="any"`) is an output validator that raises `ModelRetry` if `search_experience` was never called. `normalize_model_name` (thin wrapper over `llm/model_names.py`) maps legacy prefixes — `google_genai:`, `google-gla:`, bare `gemini-*` — onto `google:`, the name pydantic-ai uses for the Gemini API. OpenRouter runs set `openrouter_provider={"require_parameters": True}` so OpenRouter skips providers that reject our tool-call parameters. Failures are covered in two layers: `FallbackModel` for `ModelAPIError`, plus an explicit `UnexpectedModelBehavior` catch for output-retry exhaustion (raised above the model layer, where FallbackModel cannot see it).
- `reviewer.py` — `review_agent` (structured `ResumeReview` output: 1-10 relevance/quality/coherence, summary, strengths, per-section recommendations with severity, `keywords_to_add`) and `review()`, which resolves the `judge` task model through the same `_resolve_model` / `_run_with_fallback` path as the other tasks and writes its text in the requested UI language. Prompt: `prompts/review_prompt.txt`. This is the user-facing sibling of `evals/evaluators/llm_judge.py`, which only scores.
- `resume_analysis.py` — `analyze_resume()`: `ATSEvaluator` (offline; score and keyword coverage rescaled to 0-100, formatting findings as structured `issues` the frontend localizes) plus `review()` with the ATS scan passed in so the model can pick the missing keywords that matter. A review failure is contained: the ATS part is still returned with `review_error`. Backs `POST /resume/analyze-resume/{user_id}` (validates the client-supplied `resume` as a `ResumeOutputFormat`) and `POST /resume/analyze-resume-pdf/{user_id}` (multipart `file` + `job_description` + `language`; parses with `utils/resume_import.parse_resume_pdf`, maps via `utils/imported_resume.imported_to_resume`, 422 when the PDF has no work experience) in `api/routers/resume.py`. Both record a `resume_analyses` row (`DBStorage.record_resume_analysis`, attributed to `get_latest_generation_model(user_id)` for generated resumes, `source='imported'` otherwise) that `get_admin_stats()["analyses"]` aggregates.
- `agent.py::improvements_block` / `generate(improvements=...)` — "apply improvements": the review recommendations a user ticked arrive as `ResumeRequest.improvements` (plain strings, capped at `MAX_IMPROVEMENTS` x `MAX_IMPROVEMENT_CHARS`), get appended to the generation prompt as an IMPROVEMENT REQUESTS block that forbids fabrication, and are hashed into `_build_result_signature` so the regeneration bypasses the result cache while a plain request's key is unchanged. `Bot.generate_resume` / `generate_resume_progress` pass them through.
...
- `record_generation_event()` / `record_resume_analysis()` / `get_admin_stats()` power the admin dashboard (`resume_analyses` feeds the `analyses` block: count, avg ATS / reviewer score, breakdown by generation model)
...
- `evaluators/` — four scoring modules:
  ...
  - `ats_evaluator.py` — `ATSEvaluator`: keyword coverage analysis (offline); scores how well a resume matches a job description. Formatting findings come back both as the English strings the reports print (`formatting_issues`) and as structured `issues` (`FormattingIssue`: experience index + kind) for the user-facing analysis. Keyword extraction drops job-posting boilerplate and sentence-final dots. Also shared by `llm/resume_analysis.py`, so the score a user sees is the one the eval dashboard reports.
...
- `runner.py` — `EvalSpec`/`validate_spec`/`run_eval` orchestrate evaluation runs. Each cell also records the routing concessions its model needed (`unforced_tool_choice` / `allow_reasoning`, from `llm/model_routing.py`) on both the success and error paths, so a score is always read next to what it cost to get: `MAX_MODELS=5` / `MAX_CELLS=20` / `CONCURRENCY=3`. Each cell generates a resume for one (model, JD) pair, scores it, and persists immediately (failures are logged, not raised). A *judge* failure is contained separately: the generation and its schema/ATS scores are kept, the cell stays `success`, and the reason lands in `judge_error` -- `ResumeEvaluationReport` reweights schema/ATS so the composite stays meaningful (and is flagged in the UI as not comparable to a judged one). Optionally calls `on_cell` callback (used by admin `/evals` endpoint for SSE streaming). `run_eval` is shared by both the pytest integration test (`tests/integration/test_multi_model.py`) and the admin dashboard.
...
- `resume.py` — generation endpoints (generate, stream; both accept `improvements`), `POST /analyze-resume/{user_id}` (ATS score + LLM review of a generated resume) and `POST /analyze-resume-pdf/{user_id}` (same for an uploaded PDF)
