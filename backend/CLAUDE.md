...
- `tool_forcing.py` — some models reject the forced tool call our agents require (`qwen/qwen3.7-flash`'s only endpoint accepts `tool_choice: "auto"` and 404s on `"required"`). `prepare()` wraps every `openrouter:` model string in `AdaptiveToolChoiceModel`, which retries the request once with pydantic-ai's `openai_supports_tool_choice_required: False` profile flag and remembers the model for the rest of the process. Detection is adaptive rather than stored, so it self-heals across workers and covers models that never pass through the dashboard (eval runs pick arbitrary catalog models). The agent's own output validation still rejects an answer that skipped the tool.
- `model_probe.py` — `probe_model`: one minimal request in our exact shape (structured output + a function tool, plus the OpenRouter settings), run through the same `tool_forcing.prepare()` path as production. Reports three outcomes: works, works only unforced (`forced_tool_choice=False`), or does not work. The admin `PUT /model-config` runs it on the primary and fallback before storing them (rejecting only the third case); `POST /model-check` exposes it standalone. A model can advertise `tools`/`tool_choice` in the OpenRouter catalog and still refuse `tool_choice: "required"`, so a live request is the only reliable check.
