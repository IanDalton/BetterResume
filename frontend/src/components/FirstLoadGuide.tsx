...
export function FirstLoadGuide({ open, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-lg bg-neutral-900 border border-neutral-700 rounded-xl p-6 shadow-2xl space-y-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-xl font-semibold tracking-tight">{t('guide.title')}</h2>
            <p className="text-sm text-neutral-400 mt-1">{t('app.tagline')}</p>
          </div>
        ...
        <div className="space-y-3 text-sm leading-relaxed">
          ...
          <p className="text-xs text-neutral-500">{t('guide.tip')}</p>
        </div>
